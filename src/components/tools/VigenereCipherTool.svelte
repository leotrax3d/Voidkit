<script lang="ts">
  ...
  import { vigenereCipher } from '$lib/utils/ciphers';
  ...
  $: {
    try {
      outputText = vigenereCipher(inputText, keyword, mode === 'decrypt');
      errorMessage = '';
    } catch (error) {
      outputText = '';
      errorMessage = error instanceof Error ? error.message : 'Invalid keyword.';
    }
  }
</script>
...
<section class="tool-page" aria-label={tool.name}>
  ...
  <section class="panel controls">
    ...
    <input id="vigenere-keyword" type="text" bind:value={keyword} />
  </section>

  <section class="two-panel">
    <article class="panel field">
      <label for="vigenere-input">Input</label>
      <textarea id="vigenere-input" rows="9" bind:value={inputText}></textarea>
    </article>
    <article class="panel field">
      <label for="vigenere-output">Output</label>
      <textarea id="vigenere-output" rows="9" readonly value={outputText}></textarea>
    </article>
  </section>

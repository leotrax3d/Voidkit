<script lang="ts">
  ...
  import { playfairCipher } from '$lib/utils/ciphers';
  ...
  $: {
    try {
      outputText = playfairCipher(inputText, keyword, mode === 'decrypt');
      errorMessage = '';
    } catch (error) {
      outputText = '';
...
<section class="tool-page" aria-label={tool.name}>
  ...
  <section class="panel controls">
    ...
    <label for="playfair-keyword">Keyword (J merged into I)</label>
    <input id="playfair-keyword" type="text" bind:value={keyword} />
  </section>

  <section class="two-panel">
    <article class="panel field">
      <label for="playfair-input">Input</label>
      <textarea id="playfair-input" rows="9" bind:value={inputText}></textarea>
    </article>
    <article class="panel field">
      <label for="playfair-output">Output</label>
      <textarea id="playfair-output" rows="9" readonly value={outputText}></textarea>
    </article>
  </section>

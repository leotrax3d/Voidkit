...
If CI fails on arm64 with `qemu: uncaught target signal 4 (Illegal instruction)` during `npm ci`, this repository avoids arm64 emulation for install/build steps by running them on the build platform and copying runtime artifacts into the target image.
